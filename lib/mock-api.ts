import { Campaign, MediaResult, Influencer, SocialPost, Activity, DashboardStats } from './types';
import type { SocialTracker } from './types';
import { mockCampaigns, mockMediaResults, mockInfluencers, mockSocialPosts, mockActivities, mockSocialTrackers } from './mock-data';
...
export const api = {
  ...
  async createSocialTracker(tracker: Omit<SocialTracker, 'id' | 'createdAt' | 'updatedAt' | 'postsCount' | 'totalReach'>): Promise<SocialTracker> {
    await delay(800);
  ...
  async updateSocialTracker(id: string, updates: Partial<SocialTracker>): Promise<SocialTracker | null> {
    await delay(600);
  ...
  async getDashboardStats(campaignId: string): Promise<DashboardStats> {
    await delay(600);
    const mediaResults = mockMediaResults.filter(m => m.campaignId === campaignId);
    const influencers = mockInfluencers.filter(i => i.campaignId === campaignId);
    const socialPosts = mockSocialPosts.filter(s => s.campaignId === campaignId);
