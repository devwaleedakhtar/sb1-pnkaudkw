...
export default function PublicityGPTComponent() {
  ...
  const handleFiltersGenerated = useCallback(
    (filters: MediaFilters, query: string) => {
  ...
  const handleRunSearch = useCallback(
    async (filters: MediaFilters, query: string) => {
      setSearching(true);
      setCurrentFilters(filters);
      setCurrentQuery(query);
  ...
  const handleRunSavedFilter = useCallback(
    (filter: SavedFilter) => {
  ...
  const handleManualFiltersChange = useCallback((filters: MediaFilters) => {
    setCurrentFilters(filters);
  }, []);
  ...
  const handleManualRunSearch = useCallback(() => {
  ...
  }, [hasActiveSearch, currentQuery]);

  const getResultsDescription = useCallback(() => {
    if (hasActiveSearch) {
  ...
  }, [hasActiveSearch, currentFilters, mediaResults.length]);
